...
| `word/` | `officecli` 创建、校验和 HTML 渲染 | `filled-english.docx` 是使用真实项目证据填写的英文实验报告；`render/` 保留逐页 PNG。 |
| `document-from-scratch/` | `officecli` 创建、校验和 HTML 渲染 | 从零创建的英文毕业设计论文（四章结构），内容与同一项目的数据模型、页面和测试结论一致。 |
