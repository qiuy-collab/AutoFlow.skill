...
Before DELIVERY_STOP approval, set the map status to `verified` and every required validation status to `passed`. The core verifies that mapped evidence exists in `artifact_manifest.json`.
...
Every `include_paths` item in `plans/package.json` must include `requirement_ids`. `package_submission.py` refuses known secret, VCS, cache, key, and Office temporary paths. Its manifest uses `autoflow/package-manifest/1.0` and records hashes, archive names, requirement mappings, and folder/archive equality.
