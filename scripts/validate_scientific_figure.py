#!/usr/bin/env python3
"""Static preflight for AutoFlow publication-figure Python and R source files.
...
Integrated from nature-figure 2.1.0 under Apache-2.0. It remains dependency-free
and does not require nature-figure to be installed as a separate Skill.
"""
...
def render_text(path: Path, backend: str, findings: list[Finding], strict: bool) -> str:
    ...
    lines = [
        "AutoFlow Scientific Figure Static Preflight",
        f"source: {path}",
        f"backend: {backend}",
        "",
    ]
