#!/usr/bin/env python3
"""Read or write AutoFlow's default scientific-figure plotting backend.

Adapted from nature-figure 2.1.0 (Apache-2.0). The preference is kept under
AutoFlow so the integrated module does not depend on another installed Skill.
"""
...
def config_path() -> Path:
    override = os.environ.get("AUTOFLOW_FIGURE_CONFIG")
    if override:
        return Path(override).expanduser()
    return Path("~/.config/autoflow/scientific-figure.json").expanduser()
