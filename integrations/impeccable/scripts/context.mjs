...
// ─── Update check ──────────────────────────────────────────────────────────
// AutoFlow adaptation: the integrated route is offline-first. AutoFlow never
// performs a background version request or invokes the upstream updater.
...
function updateCheckDisabledByConfig(cwd = process.cwd()) {
  if (process.env.AUTOFLOW_IMPECCABLE_OFFLINE) return true;
  let value;
