...
Manual `node scripts/impeccable_adapter.mjs detect` scans use the same project filter config by default: `detector.ignoreRules`, `detector.ignoreFiles`, `detector.ignoreValues`, and `detector.designSystem.enabled`. `hook.enabled` only controls automatic hook execution, not manual CLI scans. Use `node scripts/impeccable_adapter.mjs detect --no-config ...` for a raw detector run that ignores project config/context. Use `npx impeccable ignores ...` for direct CLI CRUD on the same detector ignores.
...
2. Invoke the admin script and pass the user's output through verbatim:
   ...
   ```bash
   node integrations/impeccable/scripts/hook-admin.mjs <action> [args...]
   ```
...
```bash
node integrations/impeccable/scripts/hook-admin.mjs ignore-value overused-font Inter --shared --reason "User confirmed Inter is intentional"
```

Example intentional motion exception:

```bash
node integrations/impeccable/scripts/hook-admin.mjs ignore-value bounce-easing bounce-ball --shared --reason "User confirmed ball bounce animation is intentional"
```

Example whole-rule font exception:

```bash
node integrations/impeccable/scripts/hook-admin.mjs ignore-rule overused-font --all-values --reason "User asked to ignore overused fonts generally"
```

Example file-scoped exception:

```bash
node integrations/impeccable/scripts/hook-admin.mjs ignore-file "src/legacy/Card.tsx"
```
