...
3. 在 Skill 根目录先阅读 references/init.md。检查 Python、pip、Git、Node；安装 requirements.txt 的 Python 依赖。按需安装 Playwright 与 chromium、Mermaid CLI、D2、officecli 和 ffmpeg；默认复杂 Word 路由选择 OFFICE_WORD_BACKEND=minimaxdocx，需按该包说明安装并配置 MINIMAX_DOCX_ROOT 或 MINIMAX_DOCX_COMMAND。
...
| `minimaxdocx` / `minimax-docx` | 复杂 Word 报告/论文创建与填充后端；由 `OFFICE_WORD_BACKEND` 选择，仍由 `officecli` 校验和渲染。 |
